    ...
    for file in os.listdir(json_path):
        if file.endswith(".json"):
            with open(os.path.join(json_path, file), 'r') as f:
                ...
                # features
                listed_since = get_feature_value(
                    data["features"], "listed_since")
                year_of_construction = get_feature_value(
                    data["features"], "year_of_construction")
                living_area = get_feature_value(
                    data["features"], "living_area")
                number_of_rooms = get_feature_value(
                    data["features"], "number_of_rooms")
                number_of_bath_rooms = get_feature_value(
                    data["features"], "number_of_bath_rooms")
                number_of_stories = get_feature_value(
                    data["features"], "number_of_stories")
                photos = ", ".join(data["photos"])
                rental_price = get_feature_value(
                    data["features"], "rental_price")
                deposit = get_feature_value(data["features"], "deposit")
                rental_aggrement = get_feature_value(
                    data["features"], "rental_agreement")
                asking_price = get_feature_value(
                    data["features"], "asking_price")
                kind_of_house = get_feature_value(
                    data["features"], "kind_of_house")
                type_apartment = get_feature_value(
                    data["features"], "type_apartment")
                scrapped_at = file[:19]
                site = urlparse(url)
                ...
                cursor.execute(
                    'SELECT property_id FROM property WHERE url = ?', (url,))
